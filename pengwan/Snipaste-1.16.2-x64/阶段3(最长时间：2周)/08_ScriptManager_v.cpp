...
// nil 前置条件判断失败 1 成功 0 未找到空位 -1 未找到技能配置 -2 技能条件不符 -3 已经学会 -4 设置技能失败 -5 类型错误  check
...
// nSkillType: 1 设置主动技能等级 4 设置被动技能等级 2 学习或设置天赋等级 -1 重置技能树 -2 重置天赋树
// nil 前置条件判断失败 >0 更新前等级（+1 ） 0 设置失败 -1 未找到技能 -2 类型错误 check
...
// nil 类型错误 >0 等级 0 未找到或未学会该技能 check
...
int CScriptManager::L_CI_GetPlayerData( lua_State *L )
{
	...
	case 6:
		...
		break;
	case 7:
		lua_pushnumber(L, pPlayer->m_dwDamCount );
		break;
	case 8:
...
// 暂时屏蔽
int CScriptManager::L_KillMonster( lua_State* L )
{
	LS_CHKTOP(0);
	//if( !g_Script.m_pPlayer || lua_gettop(L)<2 )
	//	LS_CHKRET(0);

	//DWORD regionId = static_cast<DWORD>( lua_tointeger(L, 1) );
	//DWORD monsterId = static_cast<DWORD>( lua_tointeger(L,2) );

	//CRegion *pRegion = ExtraLuaFunctions::GetRegionById(regionId);
	//if (pRegion == NULL)
	//	LS_CHKRET(0);

	//CMonster *pMonster = pRegion->GetMonster(monsterId);
	//if (pMonster == NULL)
	//	LS_CHKRET(0);

	//pMonster->ModifyCurrentHP( - (int)(pMonster->m_dwHP), 0);

	//EDeadReason kill_reason = EDR_KILL;
	//if ( lua_isnumber(L, 3 ) )
	//{
	//	int reason = lua_tointeger(L, 3);
	//	if( reason>=0 && reason < EDR_MAX )
	//		kill_reason = (EDeadReason)reason;
	//}
	//	
	//pMonster->OnDead( g_Script.m_pPlayer, kill_reason);
	LS_CHKRET(0);
}
// check
int CScriptManager::L_CI_AddBuff( lua_State* L )
{
	...
	if( level <= 0  ) level = 1;
	bool ret = obj->buffManager.AddBuff( buffId, interrupt, level, 0, isForever , g_Script.m_pPlayer ? g_Script.m_pPlayer : obj );
...
static int L_CI_SendLuaMsg( lua_State *L )
{
	...
	try
	{
		...
		{
			rfalse(1,1,"[exception]SendLuaMsg error type[%d]",type);
...
static int L_CI_SetFactionInfo( lua_State *L )
{
	...
	case 11:
		...
		break;
	case 12:
		faction->byReverse = (BYTE)lua_tonumber( L ,3 );
		ret = 1;
		break;
	}
...
static int L_CI_GetFactionInfo( lua_State *L )
{
	...
	case 11:
		...
		break;
	case 12:
		lua_pushnumber( L , faction->byReverse );
		ret = 1;
		break;
	}
...
static int L_ModifyItem( lua_State *L )
{
	...
	case 'G':
		// 将指定道具生成到场景上
		{
			...
			DWORD regionId = static_cast< DWORD >( _GET_FIELD_NUMBER_DIRECTLY( 4, "regionId" ) );
			if ( regionId == 0 ){
			...
			WORD xTile = static_cast< WORD >( _GET_FIELD_NUMBER_DIRECTLY( 4 , "x" ) );
			WORD yTile = static_cast< WORD >( _GET_FIELD_NUMBER_DIRECTLY( 4 , "y" ) );
			if ( ( xTile == 0 ) || ( yTile == 0 ) ){
...
static int L_GetItemSetting( lua_State *L )
{
	...
	int			att_type	= 0;
	DWORD	att_value	= 0;
	for( att_type = 0 ; att_type < SCommAtt::CAT_MAX ; att_type ++ ) 
		if( itemData->sItemAddAtt.dwAtts[att_type] ) {
			att_value = itemData->sItemAddAtt.dwAtts[att_type];
			break;
		}

		SETTABLE_BY_STRING( "att_type", att_type  );
		SETTABLE_BY_STRING( "att_val"  , att_value );
...
#define ITEM_CONVERT reinterpret_cast< SEquipment& >( static_cast< SItemBase& >( item ) )
		...
		SETTABLE_BY_STRING( "enVal",   ( ITEM_CONVERT.enVal) );
		SETTABLE_BY_STRING( "starVal",   ( ITEM_CONVERT.starVal) );
...
#define ITEM_CONVERT reinterpret_cast< SEquipment& >( static_cast< SItemBase& >( item ) )
		...
		_IF_GET_FIELD_NUMBER( 2 , "enVal" )	ITEM_CONVERT.enVal	= ( DWORD )( __number );
		_IF_GET_FIELD_NUMBER( 2 , "starVal" )	ITEM_CONVERT.starVal	= ( DWORD )( __number );
...
#define ITEM_CONVERT reinterpret_cast< SEquipment& >( static_cast< SItemBase& >( item ) )
		...
		_IF_GET_FIELD_NUMBER( 1 , "enVal" )	ITEM_CONVERT.enVal	= ( DWORD )( __number );
		_IF_GET_FIELD_NUMBER( 1 , "starVal" )	ITEM_CONVERT.starVal	= ( DWORD )( __number );
...
static int L_GetMonsterData( lua_State *L )
{
	...
	case 35:
		...
		break;
	case 36:
		lua_pushnumber(L,mon.m_wBFCount);
		break;
	default:
		LS_CHKRET(0);
	}
...
static int L_CI_GetEquipDetails( lua_State *L )
{
	...
	SETTABLE_BY_STRING( "enVal",   ( equip->enVal) );
	SETTABLE_BY_STRING( "starVal",   ( equip->starVal) );
...
static int L_CI_UpdateEquipDetails( lua_State *L )
{
	...
	_IF_GET_FIELD_NUMBER( 2 , "enVal" )	equip->enVal	= ( DWORD )( __number );
	_IF_GET_FIELD_NUMBER( 2 , "starVal" )	equip->starVal	= ( DWORD )( __number );
...
BOOL CScriptManager::RegisterFunc()
{
	...
	//RegisterFunction( CI_GetMonsterHitList		);
