...
local function user_check(playerid)
	...
	if data[1] and  wc_data[data[1]] == nil  then--and (not IsSpanServer()) 
		data[1] = nil
...
local function _jysh_plant(playerid,seed)
   ...
   --[[NPC的id、用户名、偷的用户名，时间]] 
    ...
    look('playerid')
    look(playerid)
    wc_data[user_npc]={name = user_name,ripe =user_time+60,num = cishu,hID = user_ntype,x = user_x,y=user_y,reg = 12,tou = 0,sid= playerid}
	--getwc_data.wc_data[user_npc] = wc_data[user_npc]
	SetEvent(60,nil,"GI_jysh_time",user_npc)
    SendLuaMsg(0,{ids=msg_seed ,npc_id = user_npc},9)
end

--刷新
    --playerid表示点击着的id   ， NCP de gid  look_tree 0 >表示 查看  0 =表示刷新
local function _jysh_renovate(playerid,npc_gid)
	local wc_data = get_af_data()
	local data = user_check(playerid)
	if nil == data then
		return
